...
```
feat(wabotapi): add interactive reply buttons
fix(wabotapi): classify throttling on error.code, not HTTP status
docs(wabotapi): document the 24h window
chore(deps): bump strongo/logus to v0.4.1
```

### ⛔ Never mark a commit as a breaking change

- ❌ No `!` suffix — `feat(wabotapi)!:`
...
1. **It silently breaks the bump.** The `!` stops the type being recognised, so the
   commit falls through to a default **patch**. Measured: `feat(wabotapi)!:` shipped
...
```
fix(wabotapi): target Graph API v25.0, not v21.0

v21.0 was released 2024-10-02 and is four versions behind.
```
...
Skipped tests are treated as failures here. `bots-api-telegram`'s suite is almost entirely
`t.Skip()`, which reads as green while testing nothing. Do not inherit that.

Enable the local hooks with `git config core.hooksPath .git-hooks`.

## Verifying against Meta's docs

Do not write a wire format from memory. Two bugs in this client came from exactly
